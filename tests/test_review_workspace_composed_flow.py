...
class ReviewWorkspaceComposedFlowTests(unittest.TestCase):
    ...
    def test_exports_and_restores_review_session_without_regenerating(self) -> None:
        workspace = ReviewWorkspaceController(
            generation_action=lambda input_words: [
                OrchestratedResultGroup(
                    input_word="deliver",
        ...
        workspace.update_input_block(0, "deliver")
        workspace.generate_results()
        workspace.edit_phrase(0, 0, back_value="做演讲")
        workspace.set_phrase_selected(0, 1, False)
        workspace.set_phrase_locked(0, 0, True)
        workspace.select_deck("Default")
        workspace.state = workspace.state.__class__(
            input_blocks=workspace.state.input_blocks,
            result_groups=workspace.state.result_groups,
            available_decks=workspace.state.available_decks,
            selected_deck=workspace.state.selected_deck,
            selected_pairs_by_group=workspace.state.selected_pairs_by_group,
            locked_pairs_by_group=workspace.state.locked_pairs_by_group,
            submission_outcomes=[
                workspace._build_group_submission_outcome(
                    input_word="deliver",
                    result=SubmissionResult(
                        ...
                        failed_count=0,
                        submitted_pairs=[
                            ExtractedPhrasePair(front="deliver a speech", back="做演讲")
                        ],
                        skipped_pairs=[],
                        failed_pairs=[],
                    ),
            ...
            last_generation_duration_seconds=workspace.state.last_generation_duration_seconds,
        )

        session_payload = workspace.export_session()

        restored = ReviewWorkspaceController(
            generation_action=lambda input_words: [],
            list_decks_action=lambda: ["Default"],
            submit_action=no_op_submit_action,
        )
        restored.import_session(session_payload)

        self.assertEqual(restored.state.input_blocks[0].value, "deliver")
        self.assertEqual(
            restored.state.result_groups[0].full_ai_response, "Full response"
        )
        self.assertEqual(
            restored.state.result_groups[0].extracted_phrases[0].back, "做演讲"
        )
        self.assertFalse(restored.state.selected_pairs_by_group[0][1])
        self.assertTrue(restored.state.locked_pairs_by_group[0][0])
        self.assertEqual(restored.state.selected_deck, "Default")
        self.assertEqual(restored.state.submission_outcomes[0].submitted_count, 1)
